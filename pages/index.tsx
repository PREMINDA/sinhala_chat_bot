...
var test: MessageProps[] = [
	{ text: 'test', from: Creator.User, key: 1 },
	{
		text: 'JavaScript often abbreviated as JS, is a programming language that is one of the core technologies of the World Wide Web, alongside HTML and CSS. As of 2022, 98% of websites use JavaScript on the client side for webpage behavior, often incorporating third',
		from: Creator.Bot,
		key: 2,
	},
...
export default function Home() {
	...
	const callApi = async (input: string) => {
		...
		googleTransliterate(request, sourceText, inputLanguage, maxResult).then(function (response: any) {
		...
		<main className='bg-thembg'>
			<div className='relative max-w-4xl mx-auto h-screen flex flex-col justify-between'>
				<div className='px-4 pt-10 '>
					{message &&
						message.map((item) => {
							return <ChatMessage key={item.key} from={item.from} text={item.text} />;
						})}
